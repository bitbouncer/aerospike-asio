...
struct cl_batch_node_req 
{
    cl_batch_node_req(cl_batch_job* p_job, cl_cluster_node* p_node);
    ~cl_batch_node_req();
...
struct cl_batch_job
{
    cl_batch_job(event_base* base, ev2citrusleaf_get_many_cb user_cb, int n_digests, int timeout_ms);
    ~cl_batch_job(); 
...
static int get_many(ev2citrusleaf_cluster* cl, const char* ns, const cf_digest* digests, int n_digests, const char** bins, int n_bins, bool get_bin_data, int timeout_ms, ev2citrusleaf_get_many_cb cb, struct event_base* base);
...
static cl_batch_job* cl_batch_job_create(struct event_base* base, ev2citrusleaf_get_many_cb user_cb, int n_digests, int timeout_ms);
...
static bool cl_batch_job_add_node_unique(cl_batch_job* _this, cl_cluster_node* p_node);
static bool cl_batch_job_compile(cl_batch_job* _this, const char* ns, const cf_digest* digests, const char** bins, int n_bins, bool get_bin_data, cl_cluster_node** nodes);
...
static void cl_batch_job_node_done(cl_batch_job* _this, cl_batch_node_req* p_node_req, int node_result);
// The libevent2 timer event handler:
static void cl_batch_job_timeout_event(evutil_socket_t fd, short event,	void* pv_this);
...
int
ev2citrusleaf_get_many_digest(ev2citrusleaf_cluster* cl, const char* ns, const cf_digest* digests, int n_digests, const char** bins, int n_bins, int timeout_ms, ev2citrusleaf_get_many_cb cb, struct event_base* base)
{
	return get_many(cl, ns, digests, n_digests, bins, n_bins, true, timeout_ms,	cb, base);
}

int
ev2citrusleaf_exists_many_digest(ev2citrusleaf_cluster* cl, const char* ns, const cf_digest* digests, int n_digests, int timeout_ms, ev2citrusleaf_get_many_cb cb, struct event_base* base)
{
	return get_many(cl, ns, digests, n_digests, NULL, 0, false, timeout_ms, cb, base);
}
...
//
static int
get_many(ev2citrusleaf_cluster* cl, const char* ns, const cf_digest* digests, int n_digests, const char** bins, int n_bins, bool get_bin_data, int timeout_ms, ev2citrusleaf_get_many_cb cb, struct event_base* base)
{
	// Quick sanity check for parameters.
	if (! (cl && ns && *ns && digests && n_digests > 0 && cb && base)) 
    {
        AEROSPIKE_ERROR << "invalid parameter";
	...
	if (! nodes) 
    {
        AEROSPIKE_ERROR << "node pointer array allocation failed";
		return EV2CITRUSLEAF_FAIL_CLIENT_ERROR;
	}

	// Make a cl_batch_job object.
	cl_batch_job* p_job = cl_batch_job_create(base, cb, n_digests, timeout_ms);

	if (! p_job) {
        AEROSPIKE_ERROR << "can't create batch job";
		free(nodes);
		return EV2CITRUSLEAF_FAIL_CLIENT_ERROR;
	}
	...
	for (int i = 0; i < n_digests; i++) 
    {
		...
		nodes[i] = cl->cluster_node_get(ns, &digests[i], true);

		if (! nodes[i]) {
            AEROSPIKE_ERROR << "can't get node for digest index: " << i;
		...
		if (! cl_batch_job_add_node_unique(p_job, nodes[i])) {
            AEROSPIKE_ERROR << "can't create batch request for node:" << nodes[i]->name;
			cl_batch_job_destroy(p_job);
			free(nodes);
			return EV2CITRUSLEAF_FAIL_CLIENT_ERROR;
		}
	...
	if (! cl_batch_job_compile(p_job, ns, digests, bins, n_bins, get_bin_data,
			nodes)) {
        AEROSPIKE_ERROR << "failed batch job compile";
		cl_batch_job_destroy(p_job);
		free(nodes);
		return EV2CITRUSLEAF_FAIL_CLIENT_ERROR;
	}
	...
	if (! cl_batch_job_start(p_job)) {
        AEROSPIKE_ERROR << "failed batch job start";
		cl_batch_job_destroy(p_job);
		free(nodes);
		return EV2CITRUSLEAF_FAIL_CLIENT_ERROR;
	}
...
//
cl_batch_job::cl_batch_job(struct event_base* base, ev2citrusleaf_get_many_cb cb, int digests, int timeout_ms) :
//cross_thread_lock(NULL),
//cross_thread_locked(false),
p_event_base(base),
user_cb(cb),
n_node_reqs(0),
n_node_reqs_done(0),
...
recs(NULL),
n_recs(0),
timer_event_added(false)
{
    _timer_event_space = new uint8_t[event_get_struct_event_size()];
    // Add the timeout event right away.
    evtimer_assign((struct event*) _timer_event_space, base, cl_batch_job_timeout_event, this);

    struct timeval tv;
    tv.tv_sec = timeout_ms / 1000;
    tv.tv_usec = (timeout_ms % 1000) * 1000;

    if (0 != evtimer_add((event*)_timer_event_space, &tv)) 
    {
        AEROSPIKE_ERROR << "batch job add timer event failed";
        //throw....
        //cl_batch_job_destroy(this);
        //return NULL;
    }

    timer_event_added = true;
    size_t recs_size = n_digests * sizeof(ev2citrusleaf_rec);

    recs = (ev2citrusleaf_rec*) malloc(recs_size);

    if (!recs)
    {
        AEROSPIKE_ERROR << "batch request recs allocation failed";
        //throw..
...
cl_batch_job::~cl_batch_job()
{
    delete[] _timer_event_space;
    _timer_event_space = NULL;
}
...
//
static cl_batch_job*
cl_batch_job_create(event_base* base, ev2citrusleaf_get_many_cb user_cb, int n_digests, int timeout_ms)
{
    return new cl_batch_job(base, user_cb, n_digests, timeout_ms);
}
		...
		bool get_bin_data, cl_cluster_node** nodes)
{
	...
	for (int n = 0; n < _this->n_node_reqs; n++) {
        ...
        {
            AEROSPIKE_ERROR << "can't compile batch node request: " << n;
			return false;
		}
	}
...
cl_batch_job_start(cl_batch_job* _this)
{
	...
	for (int n = 0; n < _this->n_node_reqs; n++) {
        ...
        {
            AEROSPIKE_ERROR << "can't get fd for batch node request " << n;
			return false;
		}
	}
...
cl_batch_node_req::cl_batch_node_req(cl_batch_job* job, cl_cluster_node* node) :
p_job(job),
p_node(node),
n_digests(1),
n_recs(0),
...
cl_batch_node_req::~cl_batch_node_req()
{
    if (event_added)
        event_del((struct event*) _event_space);
    ...
    if (fd > -1) 
    {
        ...
        cf_close(fd);
        cf_atomic32_decr(&p_node->n_fds_open);
        p_node->report_failure();
        cf_atomic_int_incr(&p_node->asc->n_batch_node_timeouts);
        cf_atomic_int_incr(&p_node->asc->n_batch_node_failures);
    }

    // This balances the ref-counts we incremented in get_many().
    for (int i = 0; i < n_digests; i++) 
    {
        cl_cluster_node_put(p_node);
    }
...
{
    return new cl_batch_node_req(p_job, p_node);
}
		...
		cl_cluster_node** nodes)
{
	...
	if (! _this->wbuf) 
    {
        AEROSPIKE_ERROR << "batch node request wbuf allocation failed";
		return false;
	}
...
{
	event_assign((struct event*)_this->_event_space,
			cl_batch_job_get_base(_this->p_job), _this->fd, EV_WRITE,
			cl_batch_node_req_event, _this);

	_this->event_added = true;

	if (0 != event_add((struct event*)_this->_event_space, 0)) {
        AEROSPIKE_WARN <<"batch node request add event failed: will get partial result";
...
cl_batch_node_req_event(evutil_socket_t fd, short event, void* pv_this)
{
	...
	if (! transaction_done) {
        ...
        {
            AEROSPIKE_ERROR << "batch node request add event failed";
			cl_batch_node_req_done(_this, EV2CITRUSLEAF_FAIL_CLIENT_ERROR);
		}
	}
}
...
cl_batch_node_req_handle_send(cl_batch_node_req* _this)
{
	while(true) {
		...
		if (rv > 0) {
            ...
            AEROSPIKE_DEBUG << "send failed: fd " << _this->fd << " rv " << rv << " errno " << cf_errno();
	...
	while (true) {
		...
		if (_this->hbuf_pos < sizeof(cl_proto)) {
			...
			if (rv > 0) {
                ...
                AEROSPIKE_DEBUG << "recv connection closed: fd " << _this->fd;
				cl_batch_node_req_done(_this, EV2CITRUSLEAF_FAIL_UNKNOWN);
				return true;
			}
			...
			if (! _this->rbuf) {
				...
				if (! _this->rbuf) 
                {
                    AEROSPIKE_ERROR << "batch node request rbuf allocation failed";
...
cl_batch_node_req_parse_proto_body(cl_batch_node_req* _this, bool* p_is_last)
{
	...
	*p_is_last = false;
	...
	while (p_read < p_end) {
		...
		if (msg->data > p_end) {
            AEROSPIKE_WARN << "illegal response header format";
		...
		if (msg->info3 & CL_MSG_INFO3_LAST) {
			*p_is_last = true;
			...
			if (msg->result_code != CL_RESULT_OK || msg->data < p_end) {
                AEROSPIKE_WARN << "bad last proto body";
				return EV2CITRUSLEAF_FAIL_UNKNOWN;
			}
		...
		if (msg->result_code != CL_RESULT_OK &&
            ...
            AEROSPIKE_WARN << "batch response record result " << msg->result_code;
		...
		for (int i = 0; i < (int)msg->n_fields; i++) {
			if ((uint8_t*)(mf + 1) > p_end) {
                AEROSPIKE_WARN << "illegal response field format";
				return EV2CITRUSLEAF_FAIL_UNKNOWN;
			}
			...
			if ((uint8_t*)next_mf > p_end) {
                AEROSPIKE_WARN << "illegal response field data format";
				return EV2CITRUSLEAF_FAIL_UNKNOWN;
			}
		...
		if (! got_digest) {
            AEROSPIKE_WARN << "batch response missing digest";
			return EV2CITRUSLEAF_FAIL_UNKNOWN;
		}
		...
		if (msg->n_ops > 0) 
        {
            p_rec->bins = new ev2citrusleaf_bin[p_rec->n_bins];
            if (! p_rec->bins) 
            {
                AEROSPIKE_ERROR << "batch response bins allocation failed";
				return EV2CITRUSLEAF_FAIL_CLIENT_ERROR;
			}
		}
		...
		for (int i = 0; i < (int)msg->n_ops; i++) 
        {
            ...
            {
                AEROSPIKE_WARN << "illegal response op format";
                delete[] p_rec->bins;
				return EV2CITRUSLEAF_FAIL_UNKNOWN;
			}
            ...
            {
                AEROSPIKE_WARN << "illegal response op data format";
                delete[] p_rec->bins;
				return EV2CITRUSLEAF_FAIL_UNKNOWN;
			}
		...
		if (_this->n_recs == _this->n_digests && p_read < p_end) {
            AEROSPIKE_WARN << "got last record in batch response but there's more data";
			break;
		}
	}
