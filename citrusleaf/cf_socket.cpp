...
#ifdef DEBUG_TIME
#include <pthread.h>
static void debug_time_printf(const char* desc, int v_try, int busy, uint64_t start, uint64_t end, uint64_t deadline)
{
...
cf_socket_create_nb()
{
	...
	if (-1 == fd) {
        cf_warn("could not allocate socket, errno %d",  cf_errno());
		return -1;
	}
...
cf_socket_read_timeout(int fd, uint8_t *buf, size_t buf_len, uint64_t trans_deadline, int attempt_ms)
{
	...
	do {
		...
		if (now > deadline) {
#ifdef DEBUG_TIME
			debug_time_printf("socket readselect timeout", v_try, select_busy, start, now, deadline);
#endif
			rv = ETIMEDOUT;
			goto Out;
        }
		...
		if ((rv > 0) && cf_fd_isset(fd, rset)) {
            ...
            else if ( cf_errno() != ETIMEDOUT
                     // It's apparently possible that select() returns successfully yet
                     // the socket is not ready for reading.
                     &&  cf_errno() != EWOULDBLOCK &&  cf_errno() != EINPROGRESS &&  cf_errno() != EAGAIN) {
#ifdef DEBUG_TIME
    			debug_time_printf("socket read timeout", v_try, select_busy, start, now, deadline);
#endif
                rv =  cf_errno();
				goto Out;
            }
...
cf_socket_write_timeout(int fd, uint8_t *buf, size_t buf_len, uint64_t trans_deadline, int attempt_ms)
{
	...
	do {
		...
		if (now > deadline) {
#ifdef DEBUG_TIME
			debug_time_printf("socket writeselect timeout", v_try, select_busy, start, now, deadline);
#endif
			rv = ETIMEDOUT;
			goto Out;
        }
		...
		if ((rv > 0) && cf_fd_isset(fd, wset)) {
			...
			if (r_bytes > 0) {
				...
				// We shouldn't see 0 returned unless we v_try to write 0 bytes, which we don't.
				rv = EBADF;
				goto Out;
			}
            else if ( cf_errno() != ETIMEDOUT
                &&  cf_errno() != EWOULDBLOCK &&  cf_errno() != EINPROGRESS &&  cf_errno() != EAGAIN) {
#ifdef DEBUG_TIME
    			debug_time_printf("socket write timeout", v_try, select_busy, start, now, deadline);
#endif
                rv =  cf_errno();
                goto Out;
			}
...
cf_socket_create_nb()
{
	...
	if (-1 == fd) {
        AEROSPIKE_WARN << "could not allocate socket, errno " << cf_errno();
		return -1;
	}
	...
	if (NO_ERROR != ioctlsocket(fd, FIONBIO, &iMode)) {
        AEROSPIKE_INFO << "could not connect nonblocking socket " << fd << ", errno " << cf_errno();
		return -1;
	}
...
int
cf_socket_start_connect_nb(int fd, const sockaddr_in* sa)
{
	if (0 != connect(fd, (struct sockaddr*)sa, sizeof(*sa))) {
        if (! IS_CONNECTING()) {
            if (cf_errno() == ECONNREFUSED) 
            {
                AEROSPIKE_DEBUG << "host refused socket connection";
        	}
        	else 
            {
                AEROSPIKE_INFO << "could not connect nonblocking socket " << fd << ", errno " << cf_errno();
...
int
cf_socket_create_and_connect_nb(const sockaddr_in* sa)
{
	// Create the socket.
